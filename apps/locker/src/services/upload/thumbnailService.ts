...
async function generateImageThumbnail(file: File, fileTypeInfo: FileTypeInfo) {
...
async function generateVideoThumbnail(file: File, fileTypeInfo: FileTypeInfo) {
    let thumbnail: Uint8Array;
    try {
        addLogLine(
...
export async function generateVideoThumbnailUsingCanvas(file: File) {
    const canvas = document.createElement('canvas');
    const canvasCTX = canvas.getContext('2d');
