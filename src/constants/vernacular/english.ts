...
import { formatNumberWithCommas } from 'utils/strings';
...
const englishConstants = {
    ...
    RENEWAL_ACTIVE_SUBSCRIPTION_STATUS: (expiryTime) =>
        `Renews on ${formatDate(expiryTime)}`,
    RENEWAL_CANCELLED_SUBSCRIPTION_STATUS: (expiryTime) =>
        `Ends on ${formatDate(expiryTime)}`,
    RENEWAL_CANCELLED_SUBSCRIPTION_INFO: (expiryTime) =>
        `Your subscription will be cancelled on ${formatDate(expiryTime)}`,
    ...
    REACTIVATE_SUBSCRIPTION_MESSAGE: (expiryTime) =>
        `Once reactivated, you will be billed on ${formatDate(expiryTime)}`,
    ...
    REPLACE_THUMBNAIL_NOT_STARTED: `Some of your videos thumbnails can be compressed to save space.
