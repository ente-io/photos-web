import { getDesktopRedirectURL, getEndpoint } from 'utils/endpoint';
...
const PAYMENTS_ENDPOINT = getEndpoint('PAYMENTS');
const APP_REDIRECT_URL = getDesktopRedirectURL();
...
class billingService {
        ...
        try {
            ...
            if (!token) {
                response = await HTTPService.get(
                    `${API_ENDPOINT}/billing/plans/v2`
                );
            } else {
                response = await HTTPService.get(
                    `${API_ENDPOINT}/billing/user-plans`,
                    null,
                    {
    ...
    public async syncSubscription() {
        try {
            const response = await HTTPService.get(
                `${API_ENDPOINT}/billing/subscription`,
                null,
                {
                    'X-Auth-Token': getToken(),
                }
    ...
    public async cancelSubscription() {
        try {
            const response = await HTTPService.post(
                `${API_ENDPOINT}/billing/stripe/cancel-subscription`,
                null,
                null,
                {
                    'X-Auth-Token': getToken(),
    ...
    public async activateSubscription() {
        try {
            const response = await HTTPService.post(
                `${API_ENDPOINT}/billing/stripe/activate-subscription`,
                null,
                null,
                {
                    'X-Auth-Token': getToken(),
    ...
    public async verifySubscription(
        ...
        try {
            ...
            const response = await HTTPService.post(
                `${API_ENDPOINT}/billing/verify-subscription`,
                {
    ...
    public async redirectToPayments(
        ...
        try {
            window.location.href = `${PAYMENTS_ENDPOINT}?productID=${productID}&paymentToken=${paymentToken}&action=${action}&redirectURL=${APP_REDIRECT_URL}`;
        } catch (e) {
    ...
    public async redirectToCustomerPortal() {
        try {
            ...
            const response = await HTTPService.get(
                `${API_ENDPOINT}/billing/stripe/customer-portal`,
                { redirectURL },
                {
                    'X-Auth-Token': getToken(),
                }
