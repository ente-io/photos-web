...
export default function MLDebug() {
    ...
    const getWorker = (): ComlinkWorker<typeof DedicatedMLWorker> => {
        if (token) {
            addLogLine('Token present');
        }

        return getDedicatedMLWorker('ml-worker');
    };
    let MLWorker: ComlinkWorker<typeof DedicatedMLWorker> = useMemo(
        () => getDedicatedMLWorker('ml-worker'),
        []
    );
    ...
    const onSync = async () => {
        try {
            ...
            const mlWorker = await MLWorker.remote;
    ...
    const onLoadClusteringResults = async () => {
        try {
            ...
            const clusterPromises =
                mlLibraryData?.faceClusteringResults?.clusters
                    .map((cluster) =>
                        cluster?.slice(0, 200).map((f) => allFaces[f])
                    )
            ...
            console.log(clusteringDebugInfo);
